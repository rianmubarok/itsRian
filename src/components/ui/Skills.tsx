...
const Tag = memo(({ icon, title }: { icon: string; title: string }) => (
...
const Skills = () => {
  const [shuffledSkills, setShuffledSkills] = useState<Array<[string, string]>>(
    []
  );
    ...
    <div className="space-y-8">
      <div className="flex w-full">
        <div className="relative flex w-full flex-col justify-start gap-y-4 overflow-hidden py-2">
          {sliders}
          <div className="fade hidden dark:flex pointer-events-none bg-gradient-to-r from-primary-dark via-transparent to-primary-dark absolute inset-0" />
        </div>
      </div>
    </div>
  );
